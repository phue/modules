...
process QUALIMAP_BAMQC {
    ...
    def gcref      = meta.genome.toString().startsWith('GRCh') ? '-gd HUMAN' : meta.genome.toString().startsWith('GRCm') ? '-gd MOUSE' : ''
    def collect_pairs = meta.single_end ? '' : '--collect-overlap-pairs'

    // currently there is no convenient way to have optional input in dsl2
    // there needs to be a placeholder file in the workflow definition with the magic name 'NO_FILE' 
    def regions_file = params.regions.name != 'NOFILE' ? '--gff ${regions}' : ''
